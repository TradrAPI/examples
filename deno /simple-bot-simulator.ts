...
import { Tradr, ServerType, AccountResult, DecoratedTickItem } from 'npm:@tradrapi/trading-sdk';

const serviceUrl = 'dev' === (Deno.args[1] ?? '').toLowerCase()
    ? `https://dev.tradrapi.com`
    : 'https://live.tradrapi.com';

const socketUrl = 'dev' === (Deno.args[1] ?? '').toLowerCase()
    ? `wss://wss-dev.tradrapi.com`
    : 'wss://wss.tradrapi.com';

// Create a new instance of the SDK
const tradrApi = Tradr.make({
  exchange: { serviceUrl, socketUrl },
  auth: { apiKey: Deno.args[0] },
});

// Request the account ID to use
const ACCOUNT_ID = prompt("Enter the account ID to retrieve:");
